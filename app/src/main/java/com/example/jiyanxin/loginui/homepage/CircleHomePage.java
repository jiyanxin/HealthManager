 ...
 * 自定义绚丽的ProgressBar.
 */
public class CircleHomePage extends View {
    /**
     * 进度条所占用的角度
     */
    ...
    public CircleHomePage(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }


    public CircleHomePage(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //绘制进度条
        progressPaint.setColor(Color.parseColor(calColor(progress / max, "#ffff0000", "#ff0000ff")));
    ...
    public void setProgress(final float progress) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                float oldProgress = CircleHomePage.this.progress;
                for (int i = 1; i <= 100; i++) {
                    CircleHomePage.this.progress = oldProgress + (progress - oldProgress) * (1.0f * i / 100);
                    postInvalidate();
