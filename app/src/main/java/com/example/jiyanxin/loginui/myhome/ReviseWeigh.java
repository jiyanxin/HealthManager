...
public class ReviseWeigh extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle saveInsatnceStstes){
        ...
        setContentView(R.layout.revise_weigh);

        revise = (EditText) findViewById(R.id.revise_weigh);
        sure = (Button)findViewById(R.id.revise_sure_weigh);
    ...
    public void onClick(View v){
        if(v.getId()==R.id.revise_sure_weigh){
            ReviseSure();
        }
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Looper.prepare();
                    Toast.makeText(ReviseWeigh.this, e.getMessage(), Toast.LENGTH_SHORT).show();
                    Looper.loop();
                }
            }
        }).start();
        if (1 == result){
            Toast.makeText(ReviseWeigh.this,"修改体重成功！", Toast.LENGTH_SHORT).show();
            SharedPreferences userSettings = PreferenceManager.getDefaultSharedPreferences(this);
            SharedPreferences.Editor editor = userSettings.edit();
            editor.putString("weigh",revise.getText().toString());
            editor.commit();
        }else if(-1 == result){
            Toast.makeText(ReviseWeigh.this, "该用户名不存在！", Toast.LENGTH_SHORT).show();
        } else if(-2 == result){
            Toast.makeText(ReviseWeigh.this, "更新失败！", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private int  ReviseSureNet()throws IOException{
        ...
        String weigh = revise.getText().toString();

        if( null == weigh || weigh.length()<=0){
            Looper.prepare();
            Toast.makeText(ReviseWeigh.this,"您未输入新数据！",Toast.LENGTH_SHORT).show();
            Looper.loop();
            return 0;
        }

        String params = "uid=" + username + '&' + "weigh=" + weigh;
        String urlstr = "http://120.25.207.101/revise_weigh.php";
